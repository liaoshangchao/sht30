...
//            data_flag = 1;
        ...
        OLED_ShowNum(44,6,time[1],2,8);    //显示年
        OLED_ShowChar(56,6,'_',8);
        if ( time[2] < 10 )
        {
            OLED_ShowNum(64,6,0,1,8); 
            OLED_ShowNum(70,6,time[2],1,8);    //显示月           
        }
        else{
            OLED_ShowNum(64,6,time[2],2,8);    //显示月 
        }
        OLED_ShowChar(76,6,'_',8);
        if ( time[3] <10 )
        {
            OLED_ShowNum(84,6,0,1,16);
            OLED_ShowNum(90,6,time[3],1,8);    //显示日
        }
        else{
            OLED_ShowNum(84,6,time[3],2,8);    //显示日
        }   
...
void mcu_write_rtctime(unsigned char time[])
{
   ...
   */
    ...
    if(time[0] == 1) {
        ...
        OLED_ShowNum(16,6,time[1],2,16);    //显示年
 ...
 * @return Null
 * @note    MCU需要自行实现该功能
 * 使用说明 : 需要定义一个uint8_t全局变量last_wifi_state，用于存储上一次的WIFI状态
*******************************************************************************/
void get_wifi_status(unsigned char result)
{   
    result = mcu_get_wifi_work_state();  
    
    if ( last_wifi_state != result )   //判断WIFI状态是否改变，如果是清除状态栏
    {
